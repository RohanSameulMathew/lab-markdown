...
d={'trump': 0, 'russia': 0, 'obama': 0, 'fake news': 0, 'jobs': 0, 'china': 0, 'maga': 0, 'liberal': 0}

accumulator_length=0
num_word=0
for i in range (2009, 2019):
    file_path= '/Users/rohanmathew/Downloads/Json project/condensed_'+ str(i)+'.json'
    with open ( file_path , 'rb') as year:
        readyear=year.read()
        readyear=readyear.decode('ascii')
        datayear=json.loads(readyear)
        accumulator_length+= (len(datayear))
        for tweet in datayear:
...
print('Total number of tweets is '+ str(accumulator_length))
print ('counts= '+str(d))

sorted_d = dict(sorted(d.items()))
print(f"| {'Phrase'.center(18)} | {'Percent of Tweets'.center(20)} |")
print(f"{'-' * 20} | {'-' * 20} |")

for key, value in list(sorted_d.items()):
    percent=value*100/accumulator_length
    percent='{:05.2f}'.format(percent)
    print(f"| {key.center(18)} | {percent.center(20)} |")


terms = sorted_d.keys()
counts = sorted_d.values()
import matplotlib.pyplot as plt
fig, ax = plt.subplots()
ax.bar(terms, counts)

    
image_path = "/Users/rohanmathew/Documents/GitHub/lab-markdown/tweet_analysis.png"
fig, ax = plt.subplots()
ax.bar(terms, counts)
plt.xticks(rotation=20)
plt.ylabel("Tweet Count")
plt.title("Frequency of Keywords in Trump's Tweets")
plt.savefig(image_path)


md_output=''
md_output += f"![Tweet Analysis]({image_path})\n\n"
md_output += "This table and graph show the frequency of certain words in Trump's tweets from 2009-2018."

with open("/Users/rohanmathew/Documents/GitHub/lab-markdown/README.md", "w") as f:
    f.write(md_output)
